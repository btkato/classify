...
import { enrollStudent, cancelRegistration, listRegistrations, markAttended } from '../services/registrationService.js'
...
registrationsRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const isAdmin = req.userRoles?.some((role) => role.role === 'ADMIN') ?? false
    const registration = await markAttended(id, userId, isAdmin)
